...
int main(int argc, char const* argv[]) {
  ...
  if (argc != 4) {
    printf("\n");
    printf(
        "Using:/project/tools1/bin/deletefiles pathname matchstr timeout\n\n");

    printf(
        "Example:/project/tools1/bin/deletefiles /log/idc \"*.log.20*\" "
        "0.02\n");
    printf(
        "        /project/tools1/bin/deletefiles /tmp/idc/surfdata "
        "\"*.xml,*.json\" 0.01\n");
    printf(
        "        /project/tools1/bin/procctl 300 "
        "/project/tools1/bin/deletefiles "
        "/log/idc \"*.log.20*\" 0.02\n");
    printf(
        "        /project/tools1/bin/procctl 300 "
        "/project/tools1/bin/deletefiles "
    ...
    printf(
        "本程序把pathname目录及子目录中timeout天之前的匹配matchstr文件全部清理"
        ",timeout可以是小数。\n");
