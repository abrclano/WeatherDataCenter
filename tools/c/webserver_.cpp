...
void *thmain(void *arg)     
{
  ...
  while (true)
  {
    ...
    while (true)
    {
      ...
      if (iret==0) { logfile.Write("timout\n"); vthid[pthnum].atime=time(0); continue; }
      if (iret<0)  { logfile.Write("disconnect\n"); vthid[pthnum].atime=time(0); close(connfd); break; }

      // 如果不是GET请求报文不处理，关闭客户端的socket，继续回到循环。
      if (strncmp(strrecvbuf,"GET",3)!=0) { close(connfd); break; }
      ...
      if (Login(conn,strrecvbuf,connfd)==false) { oraconnpool.free(conn); close(connfd); break; }

      // 判断用户是否有调用接口的权限，如果没有，返回没有权限的响应报文，关闭客户端的socket，继续回到循环。
      if (CheckPerm(conn,strrecvbuf,connfd)==false) { oraconnpool.free(conn); close(connfd); break; }
      ...
      if (ExecSQL(conn,strrecvbuf,connfd)==false) { oraconnpool.free(conn); close(connfd); break; }
